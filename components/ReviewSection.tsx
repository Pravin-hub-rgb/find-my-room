...
export default function ReviewsSection({ roomId, initialReviews }: ReviewsSectionProps) {
    ...
    useEffect(() => {
        ...
        if (session?.user) {
            checkIfUserHasReviewed().then(hasReviewed => {
                if (hasReviewed) {
                    setHasReviewed(true);
                }
            });
    ...
    const checkIfUserHasReviewed = async () => {
        ...
        try {
            ...
            return !!data; // Return true if a review was found
        } catch (err) {
    ...
    const handleSubmitReview = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                // Check specifically for the duplicate review error
    ...
    const handleDeleteReview = async (reviewId: string) => {
        ...
        if (error) {
          ...
          setHasReviewed(false); // Update the `hasReviewed` state to false immediately
          fetchReviews(); // Refresh the reviews after deleting
        }
      };
      
      useEffect(() => {
        if (session?.user?.id) { // Check if session and user are available
          const userReview = reviews.find((review) => review.user_id === session.user.id);
          setHasReviewed(!!userReview); // Set it to true if the user has a review, otherwise false
        }
      }, [reviews, session]); // This will run when reviews or session changes
                    ...
                    reviews.map((review) => (
                        <div key={review.id} className="border p-3 rounded shadow-sm">
                            <div className="flex justify-between items-start">
                                ...
                                {/* Show Delete button only for the review creator */}
